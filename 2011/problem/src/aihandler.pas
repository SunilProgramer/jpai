...
type
  ...
  protected
    ...
    procedure SetEnabled(enbl: Boolean); virtual;
  public
    ...
    property Manual: Boolean read mn;
    ...
    procedure Refresh();
    procedure ManualEndAI();
    property RunDirectory: string read RunDir;
...
begin
  ...
  begin
    ...
    if not FileExists(p[i].frm.fneAI.FileName) and not (p[i].frm.cbPlayer.Checked) then
    begin
...
begin
  ...
  begin
    m.StepsLeft := m.StepsLeft - 1;
    m.StepsPassed := m.StepsPassed + 1;
  end;
  Enabled := not running;
  CalculateScores();
end;
...
procedure TAIHandler.ManualEndAI();
begin
  EndAI(self);
end;
